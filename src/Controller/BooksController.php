...
class BooksController extends AppController {
	...
	public function view($slug = null) {
		...
		$book = $this->Books->find ( 'all' )->where ( [ 
				'Books.slug' => $slug 
		] )->contain ( [ 
				'Categories',
				'Writers',
		...
		$related_book = $bookInstance->find ( 'all' )->select ( [ 
				'title',
				'image',
				'slug',
				'sale_price' 
	...
	public function add() {
		...
		$writers = $this->Books->Writers->find ( 'list', [ 
				'limit' => 200 
		] );
	...
	public function search() {
		...
		if ($this->request->is ( 'post' )) {
			...
			$bookInstance = TableRegistry::get( 'Books' );
			$books = $bookInstance->find( 'all' )->select ( [ 
					'title',
					'image',
					'slug',
					'sale_price' 
